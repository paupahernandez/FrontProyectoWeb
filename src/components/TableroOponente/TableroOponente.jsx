...
const BoardOpponent = () => {
  ...
  const handleCellClick = (row, col) => {
    ...
    const rocket = newBoardState[row][col];
    console.log("ya", newBoardState[row][col])
    if (rocket) {
      newBoardState[row][col].name = 'explosion'; // Cambia el valor de la celda a explosion
      // cambiar estado a dañado etc***
      console.log("ya2", newBoardState[row][col])
      setBoardState(newBoardState); // Actualiza el estado del tablero
    }
  };
  ...
  return (
    <div>
      <h3>Oponente: {authenticatedUsername}</h3>
      <div className="board">
        {/* Rendering the board */}
            ...
            {row.map((cell, colIndex) => {
              ...
              return (
                <div key={colIndex} className="column">
                  <Cell onClick={() => handleCellClick(rowIndex, colIndex)} rocket={rocket} />
                </div>
              );
            })}
          </div>
